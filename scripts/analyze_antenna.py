...
def analyze(s1p_path, band_key=None, plot=False, csv_out=None, verbose=True):
    ...
    # Find resonance (global minimum SWR)
    ...
    # Verdict
    verdict = swr_verdict(swr_at_resonance)
    refl = power_reflected(swr_at_resonance)
    loss = loss_db(swr_at_resonance)
    ...
    if verbose:
        ...
        print()
        print(f"  Power reflected:    {refl}%")
        print(f"  Mismatch loss:      {loss} dB")
        print()
...
def batch_analyze(directory, band_key=None, csv_out='antenna_results.csv'):
    ...
    if all_results:
        ...
        print(f"PASS: {sum(1 for r in all_results if r['verdict'] == 'PASS')}")
        print(f"PASS (note): {sum(1 for r in all_results if r['verdict'] == 'PASS (note)')}")
        print(f"MARGINAL: {sum(1 for r in all_results if r['verdict'] == 'MARGINAL')}")
        print(f"FAIL: {sum(1 for r in all_results if r['verdict'] == 'FAIL')}")
